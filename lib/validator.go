...
func RegisterRule(key string, rule Rules) (err error) {
	if CustomizeMap[key] != nil {
		return errors.New(key + " registered")
	} else {
	...
	for i := 0; i < num; i++ {
		...
		if len(roleMap[tagVal.Name]) > 0 {
			for _, v := range roleMap[tagVal.Name] {
				...
				case v == "notEmpty":
					if isBlank(val) {
						return errors.New(tagVal.Name + " value can not empty")
					}
				case strings.Split(v, "=")[0] == "regexp":
					if !regexpMatch(strings.Split(v, "=")[1], val.String()) {
						return errors.New(tagVal.Name + " format verify fail")
					}
				case compareMap[strings.Split(v, "=")[0]]:
					if !compareVerify(val, v) {
						return errors.New(tagVal.Name + " the length or value out of range," + v)
					}
				}
			}
		}
...
func VerifyMulti(st interface{}, list ...Rules) (err error) {
	for _, v := range list {
		if err := Verify(st, v); err != nil {
			return err
		}
	}
	return
